...
const filters: TranslationKey[] = ["shop.filter.all", "shop.filter.press", "shop.filter.clip", "shop.filter.print", "shop.filter.drop"];

function familyKey(product: Product): TranslationKey {
  if (product.family === "fruit") return "family.fruit";
  if (product.family === "food") return "family.food";
  if (product.family === "nature") return "family.nature";
  if (product.family === "studio") return "family.studio";
  ...
  return (
    <Link href={`/shop/${product.slug}`} className="group og-card og-hover-lift block p-3">
      ...
      <div className="px-2 pb-2 pt-4">
        <p className="mb-2 text-xs font-bold text-[color:var(--color-coral)]"><T k={familyKey(product)} /></p>
        ...
        <div className="mb-4 flex flex-wrap gap-1.5">
          {tags.map((tag) => <span key={tag} className="og-pill text-[0.68rem]">{tag}</span>)}
        </div>
    ...
    <section className="og-container py-10 md:py-16">
      <div className="og-panel mb-8 overflow-hidden p-7 md:p-10">
        <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-end">
          <div>
            <span className="og-pill mb-5 text-[color:var(--color-coral)]"><T k="shop.badge" /></span>
            <h1 className="mb-5 max-w-[11ch] text-[clamp(3rem,6vw,5.5rem)] leading-[1.02]"><T k="shop.title" /></h1>
            <p className="max-w-[48ch] text-lg leading-relaxed text-[color:var(--color-fg-muted)]"><T k="shop.body" /></p>
          </div>
      ...
      <div className="mb-6 flex flex-col justify-between gap-3 md:flex-row md:items-end">
        <div>
          <h2 className="text-3xl md:text-4xl"><T k="home.playing" /></h2>
          <p className="mt-2 text-[color:var(--color-fg-soft)]">{products.length} <T k="shop.sectionSub" /></p>
        </div>
        <span className="og-pill self-start"><T k="shop.hotFirst" /></span>
      </div>

      {products.length === 0 ? (
        <div className="soft-panel p-8"><h2 className="mb-3 text-3xl"><T k="shop.emptyTitle" /></h2><p className="text-[color:var(--color-fg-muted)]"><T k="shop.emptyBody" /></p></div>
      ) : (
